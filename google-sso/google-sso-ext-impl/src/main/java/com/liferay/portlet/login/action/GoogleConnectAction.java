...
public class GoogleConnectAction extends PortletAction {
    ...
    protected void setGoogleCredentials(HttpSession session, long companyId,
        ...
        _log.error("OpenID Connect answer:\n" + jsonObject);
    ...
    protected void addUser(HttpSession session, long companyId,
        ...
        if (pictureUrl != null) {
            UserLocalServiceUtil.updatePortrait(user.getUserId(), GoogleConnectUtil.getProfileImage(processPictureUrl(pictureUrl)));
        }
    ...
    /**
     * Helper method to remove duplicated protocol in picture url.
     *
     * @param url
     * @return
     */
    private String processPictureUrl(String url) {
        if (url == null) {
            return null;
        }
        
        return url.replaceFirst("https:https:", "https:").replaceFirst("http:http:", "http:");
    }
}
